/*
	playZ virtual classes player event handler
*/
...
PZC_fnc_handlePlayZClass={
...

	{
		...
		if( "SUV_Pink_PLAYZ" == _typePlayZ ) then {
			_x setObjectTexture [0, "PLAYZ\virtual_classnames\textures\suv_body_pink_co.paa"];
		};

		if( "SUV_Camo_PLAYZ" == _typePlayZ ) then {
			_x setObjectTexture [0, "PLAYZ\virtual_classnames\textures\suv_body_camo_co.paa"];
		};
