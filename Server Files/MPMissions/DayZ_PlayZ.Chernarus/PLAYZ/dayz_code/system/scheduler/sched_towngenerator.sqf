...
sched_townGenerator_init = {
	...
	_townarray = configFile >> format["CfgTownGenerator%1", worldName];
...
sched_townGenerator_ready = {
	...
	if( PLAYZ_worldname == "chernarus" ) then {
		...
		( (!isNil "sched_tg_state") AND {(sched_tg_state == 2)} )
			AND ({sched_tg_pos distance visiblePositionASL _character <= TRESHOLD})
	} else {
		true
	};
};
