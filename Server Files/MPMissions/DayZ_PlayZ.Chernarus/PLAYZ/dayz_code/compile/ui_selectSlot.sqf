...
if (_button == 1) then {
	...
	/* PLAYZ BASE BUILDING */
	_confM = missionConfigFile >> "cfgMagazines" >> _item;
	if (!isClass _confM) then {
		_confM = missionConfigFile >> "cfgWeapons" >> _item;
	};

	if( isClass _confM ) then {
		_name = getText(_confM >> "displayName");
	
		_cfgActions = _confM >> "ItemActions";
		_numActions = (count _cfgActions);
		_height = 0;
