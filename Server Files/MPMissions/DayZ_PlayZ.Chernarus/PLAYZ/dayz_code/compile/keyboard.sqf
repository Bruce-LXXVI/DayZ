...
if ((getPlayerUID player) in AdminList) then { // Administrators
	...
	// Scroll_KEY
	if (_dikCode == 70) then {
		_code = compile preprocessFileLineNumbers format["%1\tools\PointToDestroyPERM.sqf", EAT_directory];
		call _code;
		_handled=true;
	};
	
	// Pause_KEY
	if (_dikCode == 197) then {
		_code = compile preprocessFileLineNumbers format["%1\tools\PointToRepairPERM.sqf", EAT_directory];
		call _code;
		_handled=true;
	};
};
