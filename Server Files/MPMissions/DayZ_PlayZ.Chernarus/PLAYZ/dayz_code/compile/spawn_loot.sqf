...
switch (_iClass) do {
	...
	default {
		...
		if( count _itemTypes == 0 ) then {
			_itemTypes = ["ItemCards"];
			diag_log format["[PLAYZ] _iClass=%1 | _itemTypes=%2", _iClass, _itemTypes];
		};
		...
		//while {_qty < _max} do {
			...
			//if( _iClass == "trash" ) then {_iClass = "Trash";};

			_index = dayz_CLBase find _iClass;
			while {_index < 0} do {
				diag_log format["[PLAYZ] NOT FOUND: _iClass=%1 | _index=%2", _iClass, _index];
				//_iClass = dayz_CLBase call BIS_fnc_selectRandom;
				_iCLass = ["foodNamalsk", "civilianNamalsk", "genericNamalsk", "genericNamalskNoFood", "militaryNamalsk", "militaryNAC"] call BIS_fnc_selectRandom;
				_index = dayz_CLBase find _iClass;
				diag_log format["[PLAYZ] USING INSTEAD: _iClass=%1 | _index=%2", _iClass, _index];
			};

			_weights = [0];
			if( count dayz_CLBase > _index ) then {
			...
			while{ (count _itemTypes) < (_index + 1) } do { _itemTypes set[count _itemTypes, _itemTypes call BIS_fnc_selectRandom]; };
