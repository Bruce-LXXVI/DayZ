...
if (isClass(configFile >> "CfgWeapons" >> "Chainsaw")) then {
	...
	if (surfaceIsWater _debug) then {
		diag_log format["[ESS] surfaceIsWater = true"];
		_debug set [2,-1];
		diag_log format["[ESS] creating a Land_Misc_GContainer_Big at %1 (GPS %2)", _debug, mapGridPosition _debug];
		_platform = "Land_Misc_GContainer_Big" createVehicleLocal _debug;
		_platform setPosASL _debug;
		_platform allowDamage false;
		//_platform hideObject true;
		_platform enableSimulation false;
	...
	diag_log format["[ESS] waiting for PVDZ_plr_LoginRecord."];
	waitUntil {!isNil "PVDZ_plr_LoginRecord"};
	diag_log format["[ESS] PVDZ_plr_LoginRecord ok. Checking PVCDZ_plr_Login2 = %1", PVCDZ_plr_Login2];
	_isPZombie = false;
