...
do
{
	...
	
	{
		if(_x getVariable["isAxeAIBus",0]==1)then{	
			_getInEvent = _x addEventHandler ["GetIn", {_this execVM "PLAYZ\busroute\chernarus\player_join_bus.sqf"}];
			_x setVariable ["axeGetInBusEv",_getInEvent,false];
			_getOutEvent = _x addEventHandler ["GetOut", {_this execVM "PLAYZ\busroute\chernarus\player_leave_bus.sqf"}];
