...
dayz_CBLConfigName = format["CfgBuildingLoot%1", worldName];
if(toLower worldname == "chernarus") then {dayz_CBLConfigName = "CfgBuildingLoot";};

_config = configFile >> dayz_CBLConfigName;
for "_i" from 0 to ((count _config) - 1) do {
	...
	if (_itemCount > 0) then {
		if (dayz_CBLBase find _classname < 0) then {
			...
			dayz_CBLChances set [count dayz_CBLChances, _weighted];
			dayz_CBLBase set [count dayz_CBLBase, toLower _classname];
			diag_log format["[PLAYZ] CBLBase=%1 | CBLChances=%2", _classname, _weighted];
		};
	} else {
		...
		diag_log format["[PLAYZ] CBLBase=%1 | CBLChances=[0]", _classname];
	};
};
...
_config = configFile >> "cfgLoot";
	for "_i" from 0 to ((count (_config)) - 1) do {
		...
		dayz_CLBase set [count dayz_CLBase, toLower _classname];
		dayz_CLChances set [count dayz_CLChances, _weighted];
		diag_log format["[PLAYZ] CLBase=%1 | CLChances=%2", _classname, _weighted];
...
dayz_LootBuildings = [];
for "_i" from 0 to (count (configFile >> dayz_CBLConfigName) - 1) do {
	_type = (configFile >> dayz_CBLConfigName) select _i;
